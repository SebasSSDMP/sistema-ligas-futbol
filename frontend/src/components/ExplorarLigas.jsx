...
export default function ExplorarLigas() {
  ...
  const cargarLigas = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      if (!isMountedRef.current || requestId !== requestIdRef.current) {
        console.log(`[ExplorarLigas] cargarLigas abortado, requestId: ${requestId}, current: ${requestIdRef.current}`);
        return;
      }
  ...
  const refrescarLigas = useCallback(async () => {
    ...
    const requestId = ++requestIdRef.current;
    console.log(`[ExplorarLigas] Iniciando refrescarLigas, requestId: ${requestId}`);
    ...
    try {
      ...
      console.log(`[ExplorarLigas] ligas refresh completadas`);
    } catch (error) {
      if (!isMountedRef.current || requestId !== requestIdRef.current) return;
      console.error(`[ExplorarLigas] Error actualizando ligas:`, error);
  ...
  const seleccionarLiga = useCallback(async (liga) => {
    ...
    const requestId = ++requestIdRef.current;
    console.log(`[ExplorarLigas] Seleccionando liga: ${liga.name}, requestId: ${requestId}`);
    ...
    try {
      ...
      console.log(`[ExplorarLigas] equipos cargados: ${Array.isArray(data) ? data.length : 0}`);
    } catch (error) {
      if (!isMountedRef.current || requestId !== requestIdRef.current) return;
      console.error(`[ExplorarLigas] Error cargando equipos:`, error);
  ...
  const cargarPartidos = useCallback(async () => {
    if (!isMountedRef.current || !ligaSeleccionada?.id) {
      console.log(`[ExplorarLigas] cargarPartidos saltado: no hay liga seleccionada`);
      return;
    }

    const requestId = ++requestIdRef.current;
    console.log(`[ExplorarLigas] Cargando partidos, requestId: ${requestId}`);
    ...
    try {
      ...
      console.log(`[ExplorarLigas] partidos cargados: ${Array.isArray(data) ? data.length : 0}`);
    } catch (error) {
      if (!isMountedRef.current || requestId !== requestIdRef.current) return;
      console.error(`[ExplorarLigas] Error cargando partidos:`, error);
          ...
          <div className="mt-4 bg-dark-card rounded-xl p-4 border border-dark-border">
            <div className="flex items-center gap-6 text-sm">
              <div className="flex items-center gap-2">
                <div
                  className={`w-2 h-2 rounded-full ${cacheStatus.ligas_count > 0 ? 'bg-accent-green' : 'bg-gray-500'}`}
                ></div>
                <span className="text-gray-400">
                  Ligasy: <span className="text-white">{cacheStatus.ligas_count}</span>
                </span>
              </div>
              <div className="flex items-center gap-2">
                <div
                  className={`w-2 h-2 rounded-full ${cacheStatus.equipos_count > 0 ? 'bg-accent-green' : 'bg-gray-500'}`}
                ></div>
                <span className="text-gray-400">
                  Equipos: <span className="text-white">{cacheStatus.equipos_count}</span>
                </span>
              </div>
              <div className="flex items-center gap-2">
                <div
                  className={`w-2 h-2 rounded-full ${cacheStatus.partidos_count > 0 ? 'bg-accent-green' : 'bg-gray-500'}`}
                ></div>
                <span className="text-gray-400">
                  Partidos: <span className="text-white">{cacheStatus.partidos_count}</span>
                </span>
              </div>
              <div className="ml-auto text-gray-500 text-xs">
                TTL: Ligasy {cacheStatus.ttl_config?.ligas} | Equipos {cacheStatus.ttl_config?.equipos} | Partidos{' '}
                {cacheStatus.ttl_config?.partidos}
              </div>
            </div>
          </div>
        )}
            ...
            <div className="space-y-3 max-h-[450px] overflow-y-auto mt-4">
              ...
              {ligasFiltradas.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-400 mb-2">No se encontraron ligasy</p>
                  <button
                    onClick={() => {
                      setSearchTerm('');
              ...
              <div className="bg-dark-card rounded-2xl p-6 border border-dark-border">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    {ligaSeleccionada.logo && (
                      <img
                        src={ligaSeleccionada.logo}
                        alt={ligaSeleccionada.name}
                  ...
                  >
                    <svg
                      className={`w-4 h-4 ${ligaActualizando === ligaSeleccionada.id ? 'animate-spin' : ''}`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                ...
                <div className="flex items-center gap-4 text-sm">
                  ...
                  <span className="text-gray-400">
                    {loadingPartidos
                      ? 'Cargando partidos...'
                      : `${partidos.length} partidos${errorPartidos ? ' (error)' : ''}`}
                  </span>
                </div>
              </div>
              ...
              <div className="bg-dark-card rounded-2xl p-6 border border-dark-border">
                ...
                {loadingEquipos ? (
                  ...
                  <div className="text-center py-4">
                    ...
                    <button
                      onClick={() => seleccionarLiga(ligaSeleccionada)}
              ...
              <div className="bg-dark-card rounded-2xl p-6 border border-dark-border">
                ...
                {loadingPartidos ? (
                  ...
                  <div className="text-center py-4">
                    ...
                    <button onClick={cargarPartidos} className="text-accent-blue hover:underline text-sm">
                      Reintentar
                    </button>
                  </div>
                ) : (
